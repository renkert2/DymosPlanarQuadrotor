...
mdl_path = 'C:/Users/renkert2/Documents/ARG_Research/DymosPlanarQuadrotor' # Point Model to folder containing the Model folder.  This is required for !openmdao check functions
...
phase.set_state_options("x1", val=1, fix_initial=True) # Fix Battery State of Charge Initial State to 1
phase.set_state_options("x2", fix_initial=True, fix_final=True)
phase.set_state_options("x3", fix_initial=True, fix_final=True)

phase.set_control_options("u1", lower=0, upper=1, opt=True)
phase.set_control_options("u2", lower=0, upper=1, opt=True)

phase.add_boundary_constraint('x1_dot', loc='final', shape=(1,), equals=0) # Shape may need to change to (nn,)
phase.add_boundary_constraint('x2_dot', loc='final', shape=(1,), equals=0)

...
tx_10_dir = os.path.join(os.path.dirname(__file__), "Output_10NodeTx/")
...
graphics = plotting.timeseries_plots(prob=[cases[-1]], sim=[sim_cases[1]], phases=[f"phase{i}" for i in range(5)], title="Input Optimization, 10 Nodes", legend=None)

#%%
import weekly_reports
wdir = weekly_reports.find_dir("07062022")[-1]
figs = [x[0] for x in graphics]
fnames = [f"10nodes_mission1_{x}" for x in ["body_states", "powertrain_states", "inverter_currents", "inverter_inputs"]]

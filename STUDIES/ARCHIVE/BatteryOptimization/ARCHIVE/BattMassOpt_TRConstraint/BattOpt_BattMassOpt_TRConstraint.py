...
if not os.path.isdir("Output_BattMassOpt_TRConstraint"):
    os.mkdir("Output_BattMassOpt_TRConstraint")
os.chdir("Output_BattMassOpt_TRConstraint")
...
def setup_prob(opt_type):
    ...
    phase.set_time_options(fix_initial=True, initial_val=0, duration_bounds=(1, 100), duration_ref0=1, duration_ref=100)
    ...
    # Add a constraint on the Thrust Ratio
    if opt_type == "time":
        planar_model.add_constraint('thrust_ratio.TR', lower=1.1, upper=5)
    elif opt_type == "payload":
        planar_model.add_constraint('thrust_ratio.TR', lower=1.0, upper=1.1)
    ...
    recorder = om.SqliteRecorder(f'cases_{opt_type}.sql', record_viewer_data=False)
    ...
    phase.add_objective('time', loc='final', scaler=None)
...
def set_vals(prob, phase):
    ...
    prob.set_val('traj.phase0.parameters:PT_theta16', 1.00)
    
sims = {}

#%% Minimize Time
(prob, phase) = setup_prob("time")
set_vals(prob, phase)
prob.run_driver()
prob.record('final')
prob.cleanup()
sims["time"] = prob.model.traj.simulate(times_per_seg=50)

#%% Maximize Mass
(prob, phase) = setup_prob("payload")
set_vals(prob, phase)
prob.run_driver()
prob.record('final')
prob.cleanup()
sims["payload"] = prob.model.traj.simulate(times_per_seg=50)


#%% Run the optimization with the BatteryMass optimization variable
cr_time = om.CaseReader('cases_time.sql')
cr_payload = om.CaseReader('cases_payload.sql')

cases = {"time": cr_time.get_case('final'), "payload":cr_payload.get_case('final')}

#%% Get the final times

tr = cases["time"]["thrust_ratio.TR"]
my_plt.subplots(sims["time"], cases["time"], path='traj.phase0.timeseries',
             vars=[f"states:{x}" for x in  ['BM_x', 'BM_y', 'BM_theta']] + [f"controls:{x}" for x in  ['PT_u1', 'PT_u2']],
             labels=['$x$', '$y$', r'$\theta$', "$u_1$", "$u_2$"], 
             title=f"Input Optimization: $TR = {tr}$", save=True)

tr = cases["payload"]["thrust_ratio.TR"]
my_plt.subplots(sims["payload"], cases["payload"], path='traj.phase0.timeseries',
             vars=[f"states:{x}" for x in  ['BM_x', 'BM_y', 'BM_theta']] + [f"controls:{x}" for x in  ['PT_u1', 'PT_u2']],
             labels=['$x$', '$y$', r'$\theta$', "$u_1$", "$u_2$"], 
             title=f"Input Optimization: $TR = {tr}$", save=True)

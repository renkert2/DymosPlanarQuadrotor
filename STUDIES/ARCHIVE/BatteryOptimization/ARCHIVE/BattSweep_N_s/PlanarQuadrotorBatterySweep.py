...
def main():
    ...
    attach_recorder(prob, "BattSweep_3S_4000.sql")
    prob.set_val('N_s__Battery', val=3)
    prob.set_val('Q__Battery', val=4000)
    run_problem(prob)
    prob.record('final')
    case_1_time = prob.get_val('traj.phase0.timeseries.time')[-1]
    print("Case 1 N_s: " + str(prob.get_val('N_s__Battery')))
    print("Case 1 Time: " + str(case_1_time))
    prob.cleanup()
    
    # Case 2
    # prob = initialize(prob, phase)
    # attach_recorder(prob, "BattSweep_6S_4000.sql")
    # prob.set_val('N_s__Battery', val=6)
    # prob.set_val('Q__Battery', val=4000)
    # run_problem(prob)
    # prob.record('final')
    # case_2_time = prob.get_val('traj.phase0.timeseries.time')[-1]
    # print("Case 2 N_s: " + str(prob.get_val('N_s__Battery')))
